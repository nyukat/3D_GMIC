...
def crop_pytorch_3d(original_img_pytorch, crop_shape, crop_position, out,
    ...
    :param method: supported in ["center", "upper_left"]
    :return: (N, K, C, h, w) PyTorch Tensor
    """
    # retrieve inputs
    num_slices_per_patch, H, W = original_img_pytorch.shape
    crop_x, crop_y = crop_position
    x_delta, y_delta = crop_shape
...
def get_max_window_3d(input_image, window_shape, pooling_logic="avg", return_max_values=False):
    """
    ...
    if pooling_logic == "avg":
        ...
        sliding_windows = F.unfold(input_image, kernel_size=window_shape).view(N, C, window_shape[0] * window_shape[1],
                                                                               -1)
        ...
        if pooling_logic == "std":
            ...
            agg_res = -sliding_windows * torch.log(sliding_windows) - (1 - sliding_windows) * torch.log(
                1 - sliding_windows)
...
def generate_mask_uplft_3d(input_image, window_shape, upper_left_points, use_gpu=False, half=False):
    ...
    if half:
        return 1 - selected.half()
    else:
        return 1 - selected.float()